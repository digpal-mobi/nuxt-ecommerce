...
import { FOOTER_NAVIGATION } from '~/Data/DummyFooterNavigation';
...
const handleSubscribe = () => {
  if (email.value.trim()) {
    console.log('Newsletter subscription email:', email.value);
    email.value = '';
  }
};
</script>
...
<template>
  <footer class="mt-[100px] laptop:mt-[150px]">
    <!-- Newsletter Card (Overlapping Banner) -->
    <MainContainer>
      <Container class="mb-[-99px] !py-[0px] px-[16px] laptop:mb-[-85px] laptop:px-[100px] desktop-lg:mb-[-90px]">
        <div class="relative">
          <div
            class="section-sub-footer relative z-[1] flex flex-col rounded-[20px] bg-black px-[24px] py-[32px] text-center laptop:flex-row laptop:items-center laptop:justify-between laptop:px-[64px] laptop:py-[36px] laptop:text-left"
          >
            <div class="mb-[24px] laptop:mb-0 laptop:flex-[0_0_528px] desktop:flex-[0_0_628px] desktop-lg:flex-[0_0_728px]">
              <h2 class="font-integral text-[32px] font-[700] leading-[38px] text-white tablet:text-[40px] tablet:leading-[46px]">
                STAY UPTO DATE ABOUT OUR LATEST OFFERS
              </h2>
            </div>

            <form
              class="flex w-full flex-col gap-4 laptop:max-w-[349px]"
              @submit.prevent="handleSubscribe"
            >
              <!-- Email Input -->
              <div class="relative w-full">
                <span class="pointer-events-none absolute left-5 top-1/2 -translate-y-1/2 text-black/40">
                ...
                <input
                  ...
                  class="w-full rounded-full border border-[#E5E5E5] bg-white py-3 pl-14 pr-5 text-[16px] font-medium text-black placeholder:text-black/40 focus:outline-none"
                />
              </div>

              <button
                type="submit"
                class="w-full cursor-pointer rounded-full bg-white py-3 text-[16px] font-medium text-black transition hover:bg-gray-100"
              >
    ...
    <!-- Main Footer Body -->
    <MainContainer class="bg-[#F0F0F0]">
      <Container class="bg-[#F0F0F0] px-[16px] pt-[140px] laptop:px-[100px] laptop:!pt-[150px]">
        <div class="flex flex-col gap-12 laptop:flex-row laptop:items-start">
          <!-- Left Brand & Social Info -->
          <div class="w-full laptop:max-w-[250px] desktop:max-w-[260px]">
            <h2 class="text-[32px] font-black leading-none text-black">
              SHOP.CO
            </h2>

            <p class="mt-[14px] font-satoshi text-[14px] font-[400] leading-[22px] text-black/60 laptop:mt-[25px]">
              We have clothes that suits your style and which you're proud to wear. From women to men.
            </p>

            <!-- Social Links -->
            <div class="mt-[20px] flex items-center gap-3 laptop:mt-[35px]">
              <!-- Twitter / X -->
              <NuxtLink
                to="#"
                rel="noopener noreferrer"
                target="_blank"
                class="flex h-[32px] w-[32px] items-center justify-center rounded-full border border-black/20 bg-white text-black transition hover:bg-black hover:text-white"
                aria-label="Twitter"
              >
          ...
          <div class="grid flex-1 grid-cols-2 gap-y-8 tablet:grid-cols-4 laptop:gap-x-10 desktop:gap-x-16">
            <div v-for="section in FOOTER_NAVIGATION" :key="section.id">
              <h3 class="mb-6 text-[14px] font-semibold uppercase tracking-[3px] text-black">
                {{ section.title }}
              </h3>
              ...
              <ul class="space-y-4">
                <li v-for="item in section.items" :key="item.href">
                  <NuxtLink
                    :to="item.href"
                    class="text-[15px] text-[#666666] transition hover:text-black"
                  >
                    {{ item.title }}
                  </NuxtLink>
                </li>
        ...
        <!-- Divider -->
        <div class="mb-[25px] mt-[50px] h-px bg-[#D9D9D9]" />

        <!-- Bottom Row (Copyright & Payment Badges) -->
        <div class="flex w-full flex-col items-center justify-between gap-6 pb-[80px] tablet:flex-row">
          <p class="w-full text-center text-[14px] text-[#666666] tablet:text-left">
          ...
          <div class="flex items-center justify-end gap-[12px]">
            ...
            <div class="flex h-[30px] w-[46px] items-center justify-center rounded-[5px] border border-gray-200 bg-white px-2 shadow-2xs">
              <svg width="29" height="12" viewBox="0 0 29 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M13.3296 8.75346H12.2811V0.595211H15.0606C15.765 0.595211 16.3657 0.830021 16.8572 1.29964C17.3595 1.76926 17.6107 2.34263 17.6107 3.01975C17.6107 3.71326 17.3595 4.28663 16.8572 4.75079C16.3712 5.21494 15.7705 5.44429 15.0606 5.44429H13.3296V8.75346ZM13.3296 1.59998V4.44499H15.0824C15.4974 4.44499 15.8469 4.30301 16.12 4.02452C16.3985 3.74602 16.5404 3.40746 16.5404 3.02521C16.5404 2.64842 16.3985 2.31533 16.12 2.03683C15.8469 1.74741 15.5029 1.60544 15.0824 1.60544H13.3296V1.59998Z" fill="#3C4043" />
                <path d="M20.3519 2.98699C21.1273 2.98699 21.7389 3.19449 22.1866 3.6095C22.6344 4.02452 22.8583 4.59243 22.8583 5.31324V8.75346H21.859V7.97804H21.8153C21.3839 8.61694 20.8051 8.93366 20.0843 8.93366C19.4672 8.93366 18.9539 8.75346 18.5389 8.3876C18.1239 8.02173 17.9164 7.56849 17.9164 7.02243C17.9164 6.44359 18.1348 5.9849 18.5717 5.64634C19.0085 5.30231 19.5928 5.13303 20.3191 5.13303C20.9416 5.13303 21.4549 5.24771 21.8535 5.47706V5.23678C21.8535 4.87092 21.7116 4.56512 21.4221 4.30847C21.1327 4.05182 20.7942 3.92622 20.4065 3.92622C19.8222 3.92622 19.358 4.17195 19.0195 4.66888L18.0966 4.09004C18.6045 3.35285 19.358 2.98699 20.3519 2.98699ZM18.9976 7.03881C18.9976 7.31184 19.1123 7.54119 19.3471 7.72139C19.5764 7.90159 19.8495 7.99443 20.1607 7.99443C20.6031 7.99443 20.9962 7.83061 21.3402 7.50297C21.6843 7.17532 21.859 6.79308 21.859 6.35076C21.5314 6.09411 21.0781 5.96306 20.4938 5.96306C20.0679 5.96306 19.713 6.06681 19.429 6.26885C19.1396 6.48182 18.9976 6.73847 18.9976 7.03881Z" fill="#3C4043" />
                <path d="M28.5593 3.16724L25.0645 11.2054H23.9832L25.2829 8.39311L22.9785 3.16724H24.1198L25.7798 7.17538H25.8016L27.418 3.16724H28.5593Z" fill="#3C4043" />
                <path d="M9.1592 4.78355C9.1592 4.44172 9.12862 4.11462 9.07183 3.80009H4.6771V5.60211L7.20812 5.60265C7.10545 6.20224 6.77508 6.71336 6.26888 7.0541V8.22323H7.77548C8.65519 7.40905 9.1592 6.20551 9.1592 4.78355Z" fill="#4285F4" />
                <path d="M6.26942 7.05416C5.85004 7.33702 5.30998 7.50248 4.67818 7.50248C3.45773 7.50248 2.42239 6.6801 2.05161 5.57158H0.497511V6.7773C1.26746 8.3052 2.84996 9.35365 4.67818 9.35365C5.94178 9.35365 7.00333 8.93809 7.77601 8.22274L6.26942 7.05416Z" fill="#34A853" />
                <path d="M1.90522 4.67706C1.90522 4.3658 1.9571 4.06492 2.05156 3.78206V2.57634H0.497464C0.179109 3.20814 0 3.9213 0 4.67706C0 5.43282 0.179655 6.14598 0.497464 6.77778L2.05156 5.57206C1.9571 5.2892 1.90522 4.98832 1.90522 4.67706Z" fill="#FABB05" />
                <path d="M4.67818 1.85117C5.36786 1.85117 5.98546 2.08871 6.4731 2.55287L7.80823 1.21882C6.99732 0.463612 5.94014 0 4.67818 0C2.8505 0 1.26742 1.04844 0.497464 2.57634L2.05156 3.78206C2.42234 2.67354 3.45773 1.85117 4.67818 1.85117Z" fill="#E94235" />
              </svg>
            </div>
          </div>
        </div>
